...
import VistoriaOS from './VistoriaOS'
...
function DetalhesOS({ os: osInicial, onVoltar, onAtualizar, onElaborarRM, onVistoria }) {
  ...
  return (
    <div style={{ minHeight:'100vh', background:'#2D3A8C', fontFamily:'system-ui,sans-serif', display:'flex', flexDirection:'column' }}>
      ...
      <div style={{ flex:1, background:'#E6F1FB', padding:16, overflowY:'auto' }}>
        ...
        <button onClick={() => onVistoria && onVistoria(os)}
          style={{ width:'100%', padding:13, background:'#1A6B4A', color:'#fff', border:'none', borderRadius:12, fontSize:15, fontWeight:500, cursor:'pointer', borderBottom:'3px solid #0F4A32', marginBottom:8 }}>
          Fazer Vistoria
        </button>
...
function PainelAdm({ perfil, onLogout }) {
  ...
  const [osVistoria, setOsVistoria] = useState(null)
  ...
  if (tela === 'vistoria') return <VistoriaOS os={osVistoria} usuarioId={perfil.id} onFechar={() => { setTela('detalhes'); setTelaParams(osVistoria) }} />
  ...
  if (tela === 'detalhes') return <DetalhesOS os={telaParams} onVoltar={() => setTela('lista')} onAtualizar={() => setTela('home')} onElaborarRM={(os) => { setTelaParams(os); setTela('elaborar_rm') }} onVistoria={(os) => { setOsVistoria(os); setTela('vistoria') }} />
