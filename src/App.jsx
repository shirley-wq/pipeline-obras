...
function SidebarRH({ ativa, onChange, totalColaboradores, totalHolerites }) {
  const itens = [
    ...
    { id:'horas_extras', label:'Resumo de Holerite', count:null },
  ]
  return (
...
// "RM Enviada" nunca fica parada de verdade nessas réguas - salvarStatus() já empurra
// automaticamente pra "Aguardando Pedido"/"Aguardando Pedido da Tecban" no mesmo salvamento (nunca
// existe uma obra com status = RM ENVIADA persistido aqui). Na régua isso aparecia como duas
// bolinhas que sempre "andam" juntas, dando sensação de redundância - Shirley pediu pra juntar numa
// bolinha só (2026-09-04). Não mexe nos status reais gravados no banco, só em como a régua agrupa
// pra exibir - as duas etapas originais continuam existindo e sendo usadas normalmente em todo o
// resto do sistema (filtros, grupos, "Erro no pedido", dias parado etc.).
function agruparEtapasVisuais(etapas) {
  const grupos = []
  for (let i = 0; i < etapas.length; i++) {
    if (etapas[i] === 'RM ENVIADA' && etapas[i + 1] && etapas[i + 1].startsWith('AGUARDANDO PEDIDO')) {
      grupos.push({ label: 'RM Enviada / Aguardando Pedido', indices: [i, i + 1] })
      i++
    } else {
      grupos.push({ label: etapas[i], indices: [i] })
    }
  }
  return grupos
}
...
function Regua({ tipo, rede, status, lembretes, onRemoverLembrete }) {
  ...
  const grupos = agruparEtapasVisuais(etapas)
  ...
  return (
    <div style={{ display:'flex', alignItems:'flex-start', padding:'10px 0 6px', overflowX:'auto', gap:0 }}>
      {grupos.map((grupo, gi) => {
        const nums = grupo.indices.map(idx => idx + 1)
        const concluida = Math.max(...nums) < atual
        const ativa = nums.includes(atual)
        const lembretesAqui = lista.filter(l => nums.includes(Number(l.etapa)))
        const cor = concluida ? '#1A6B4A' : ativa ? '#2D3A8C' : '#D1D5DB'
        return (
          <div key={gi} style={{ flex:1, minWidth:48, display:'flex', flexDirection:'column', alignItems:'center', position:'relative' }}>
            {gi < grupos.length - 1 && (
            ...
            <div style={{ fontSize:8, color: concluida ? '#1A6B4A' : ativa ? '#2D3A8C' : '#9CA3AF', marginTop:4, textAlign:'center', lineHeight:1.2, maxWidth:48 }}>{grupo.label}</div>
...
export default function App() {
  ...
  const contasPagarVencemHoje = contasPagar.filter(c => c.data_vencimento === dataHojeIso && (c.status_pagamento || 'pendente') === 'pendente')
  ...
  return (
    <div style={estilo}>
      ...
      {/* ====== ABA: INDISPONÍVEL PARA FATURAR ====== */}
      {aba === 'indisponivel' && (
        <div style={{ padding:12 }}>
          <input value={buscaIndisponivel} onChange={e=>setBuscaIndisponivel(e.target.value)} placeholder="🔎 Buscar por nome, PC, SIGE, OS, pedido, cidade..." title="Busca"
          ...
          {obrasIndisponiveis.length === 0 ? (
            <div style={{ textAlign:'center', color:'#EA580C', marginTop:40, fontSize:14 }}>{buscaIndisponivel ? 'Nenhuma obra encontrada' : 'Nenhuma obra com pedido pendente de conferência'}</div>
          ) : (
            <>
              <div style={{ fontSize:11, color:'#9A3412', fontWeight:700, marginBottom:10, padding:'8px 12px', background:'#FFF7ED', borderRadius:8, border:'1px solid #FED7AA' }}>
                {obrasIndisponiveis.length} obra(s) com pedido recebido, mas ainda sem a conferência de valor/OS/CNPJ
              </div>
              {obrasIndisponiveis.map(o => {
                const tc = TIPO_COR[o.tipo] || { bg:'#F1F5F9', text:'#475569' }
                const sc = STATUS_COR[o.status] || { bg:'#F1F5F9', text:'#475569' }
                return (
                  <div key={o.id} style={{ background:'#fff', border:'1px solid #FED7AA', borderRadius:12, marginBottom:10, padding:'12px 14px' }}>
                    ...
                    <div style={{ fontSize:11, background:'#FFF7ED', borderLeft:'3px solid #EA580C', padding:'5px 8px', borderRadius:4, color:'#9A3412', marginTop:8 }}>
                      📋 Falta o PDF do pedido ser processado (sem valor/OS/CNPJ conferidos) — alimente na pasta "PEDIDOS IA" pra rodar a conferência de verdade.
                    </div>
                  </div>
                )
              })}
